...
class hashset {
	typedef hashset<T, Hashcode> my_type;
	...
	void rehash() {
		...
		for (int j = 0; j < oldtblsize; j++)
			for (backet_type::const_iterator backetit = oldbackets[j].begin();
					...
					++backetit)
				c[hashcode(*backetit) % tblsize].push_back(*backetit);
...
public:
	...
	hashset(key_hashcode inHashcode = key_hashcode(), size_type inTblsize = 17, float inLoad = 0.75f) : hashcode(inHashcode), load(inLoad), tblsize(inTblsize), length(0) {
		c = new backet_type [tblsize];
	}
	hashset(const my_type& x, key_hashcode inHashcode = key_hashcode(), size_type inTblsize = 17, float inLoad = 0.75f) : hashcode(inHashcode), load(inLoad), tblsize(inTblsize), length(0) {
		c = new backet_type [tblsize];
